...
1. **Models**  
   Contains data models and DTOs for weather information.

2. **Services**  
   Implements core business logic such as fetching weather data from OpenWeather API.

3. **Web**  
   ASP.NET Core web application providing a dashboard for users to search cities, view current weather, air quality.

4. **WiraSystemOpenWeather**  
   REST API project exposing endpoints for weather data, supporting city queries, pollutants, and AQI information.

5. **WiraWeatherTest**  
   Unit and integration tests for service to ensure correctness and response validation.
